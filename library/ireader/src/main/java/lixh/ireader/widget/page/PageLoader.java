...
import com.lixh.utils.IOUtils;
import com.lixh.utils.RxUtils;
...
public abstract class PageLoader<T> {
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = DensityUtil.dpToPx(DEFAULT_MARGIN_WIDTH);
        mMarginHeight = DensityUtil.dpToPx(DEFAULT_MARGIN_HEIGHT);
        // 配置文字有关的参数
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize = textSize;
        mTitleSize = mTextSize + DensityUtil.sp2px(EXTRA_TITLE_SIZE);
        // 行间距(大小为字体的一半)
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(DensityUtil.sp2px(DEFAULT_TIP_SIZE)); // Tip默认的字体大小
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = DensityUtil.dpToPx(3);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + DensityUtil.dpToPx(2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = DensityUtil.dpToPx(6);
        int polarWidth = DensityUtil.dpToPx(2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - DensityUtil.dpToPx(2));
        ...
        int outFrameBottom = visibleBottom - DensityUtil.dpToPx(2);
        ...
        float x = outFrameLeft - mTipPaint.measureText(time) - DensityUtil.dpToPx(4);
